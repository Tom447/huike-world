...
//    public int countAllCluesForIndex(IndexStatisticsVo indexStatisticsVo);
     ...
     * 根据下次跟进时间查询
     */
    @Select("select * from tb_clue where status = #{status} and next_time >= #{begin} and next_time <= #{end}")
    List<TbClue> selectByNextTime(LocalDateTime begin, LocalDateTime end, String status);



    List<ClueCountDTO> countByEveryDay(LocalDateTime beginTime, LocalDateTime endTime);
}
