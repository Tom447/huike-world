...
public interface TbAssignRecordMapper
{
   ...
   // public int countAssignByUser(@Param("userId") Long userId, @Param("type") String type);
    ...
    /**
     * 根据线索的ID/商机的ID查询对应的分配人
     */
    @Select("select u.* from tb_assign_record ar , sys_user u where ar.user_id = u.user_id and  assign_id = #{assignId} and type = #{type} and latest = '1'")
    SysUser selectByAssignIdAndType(Long assignId, String type);
