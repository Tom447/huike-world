...
public interface TbBusinessMapper {
     ...
     * 根据nextTime查询符合条件的商机
     */
    @Select("select * from tb_business where status = #{status} and next_time >= #{begin} and next_time <= #{end}")
    List<TbBusiness> selectByNextTime(LocalDateTime begin, LocalDateTime end, String status);


    List<BusinessCountDTO> countByEveryDay(LocalDateTime beginTime, LocalDateTime endTime);
}
